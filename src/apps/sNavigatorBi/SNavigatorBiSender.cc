...
#include "apps/sNavigatorBi/SNavigatorBiSender.h"

#define round(x) floor((x) + 0.5)

Define_Module(SNavigatorBiSender);
using namespace inet;

SNavigatorBiSender::SNavigatorBiSender()
{
    selfSource_ = nullptr;
    selfSender_ = nullptr;
}
...
void SNavigatorBiSender::initialize(int stage)
{
...
void SNavigatorBiSender::handleMessage(cMessage *msg)
{
    if (msg->isSelfMessage())
    {
        if (!strcmp(msg->getName(), "selfSender"))
            sendsNavigatorBiPacket();
...
void SNavigatorBiSender::initTraffic()
{
...
void SNavigatorBiSender::selectPeriodTime()
{
...
void SNavigatorBiSender::sendsNavigatorBiPacket()
{
    ...
    auto sNavigator = makeShared<SNavigatorBiPacket>();
